...
int e2fsck_lfsck_check_lustre_version(char *buf, int buflen)
{
	FILE *f;
	int maj, min, rel;
	int rc = 0;

	f = fopen("/proc/fs/lustre/version", "r");
	if (f == NULL)
		return 0;

	/* /proc/fs/lustre/version should have the first line like:
	 *
	 * lustre: 2.5.55
	 *
	 * and optionally some other lines after it, which we ignore */
	if (fgets(buf, buflen, f) == NULL) {
		rc = 0;
	} else if (sscanf(buf, "lustre: %u.%u.%u", &maj, &min, &rel) != 3) {
		rc = 1;
	} else if (maj >= 3 || (maj == 2 && min >= 6) ||
		   (maj == 2 && min == 5 && rel >= 55)) {
		rc = 1;
	}
...
/* If the new in-kernel LFSCK has been run on this filesystem to
 * repair OST orphans, we shouldn't be running this lfsck anymore. */
int e2fsck_lfsck_check_layout(e2fsck_t ctx, char *block_buf)
{
	ext2_filsys fs = ctx->fs;
	ext2_ino_t  tinode;
	int rc = 0;

	rc = ext2fs_lookup(fs, EXT2_ROOT_INO, LFSCK_LAYOUT,
			   strlen(LFSCK_LAYOUT), block_buf, &tinode);
	if (rc == 0) {
...
{
	/* if we find a DNE LMA xattr, then we shouldn't be running on
	 * this filesystem at all. */
	if ((lma->lma_incompat & ~LMA_INCOMPAT_SUPP) != 0) {
		VERBOSE(ctx, "error: do not handle lma_incompat %08x\n",
