...
int lfsck_get_fid(ext2_filsys fs, ino_t ino, struct lu_fid *fid)
{
	...
	errcode_t rc;
	char buf[LMA_OLD_SIZE];
	struct lustre_mdt_attrs *lma = (struct lustre_mdt_attrs *)buf;
	int size;
